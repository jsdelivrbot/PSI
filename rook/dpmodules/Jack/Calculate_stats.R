	...
	#	}
		...
		error <- FALSE
		 if(stat=="mean"){
			...
			print("i think the error is here")
			print(var)
			print(class(var))
			if(class(var)=="list"){
				var <- unlist(var)
			}
			out <- dpMean$new(mechanism='mechanismLaplace', var.type=type, variable=var, n=n, rng=rng, epsilon=eps_i, impute.rng=impute.rng, alpha=Beta)
			print("cleared out")
			col <-  which(colnames(data) == var)
			input <- data[,col]
		}
